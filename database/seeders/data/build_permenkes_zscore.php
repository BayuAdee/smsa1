...
/**
 * Script Builder Data Referensi Z-Score Resmi Permenkes RI No. 2 Tahun 2020
 * Menggunakan Tabel 1, 2, 3, 8, 9, 10 dari Permenkes 2020 (BB/U & TB/U 0-60 Bulan)
 */
$headers = ['indicator', 'jenis_kelamin', 'usia_bulan', 'l', 'm', 's', 'sd3neg', 'sd2neg', 'sd1neg', 'sd0', 'sd1', 'sd2', 'sd3'];
$rows = [$headers];

// 1. Standar Berat Badan menurut Umur (BB/U) Laki-Laki 0-60 Bulan (Tabel 1 Permenkes 2020)
$bbu_l = [
...
// 2. Standar Berat Badan menurut Umur (BB/U) Perempuan 0-60 Bulan (Tabel 8 Permenkes 2020)
$bbu_p = [
...
// 3. Standar Panjang/Tinggi Badan menurut Umur (TB/U) Laki-Laki 0-60 Bulan (Tabel 2 & 3 Permenkes 2020)
$tbu_l = [
    ...
    24 => [78.0, 81.0, 84.1, 87.1, 90.2, 93.2, 96.3], // Menggunakan Tabel 3 untuk umur >= 24 bulan
...
// 4. Standar Panjang/Tinggi Badan menurut Umur (TB/U) Perempuan 0-60 Bulan (Tabel 9 & 10 Permenkes 2020)
$tbu_p = [
    ...
    24 => [76.0, 79.3, 82.5, 85.7, 88.9, 92.2, 95.4], // Menggunakan Tabel 10 untuk umur >= 24 bulan
...
foreach ($configs as $cfg) {
    [$ind, $jk, $dataset, $l_val] = $cfg;
    for ($age = 0; $age <= 60; $age++) {
        $sds = $dataset[$age];
        [$sd3neg, $sd2neg, $sd1neg, $sd0, $sd1, $sd2, $sd3] = $sds;

        $m_val = $sd0;

        // S_val = (SD1 - SD-1) / (2 * M)
        $s_val = round(($sd1 - $sd1neg) / (2 * $m_val), 4);
...
$csvPath = __DIR__.'/zscore_references.csv';
$fp = fopen($csvPath, 'w');
foreach ($rows as $row) {
    fputcsv($fp, $row);
}
...
echo 'Berhasil memperbarui zscore_references.csv berbasis Permenkes 2020 ('.(count($rows) - 1)." baris).\n";
