...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
    ...
    <div class="bg-gradient-to-br from-slate-900 to-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-300">
        <div class="flex items-center justify-between">
            ...
            <div class="w-14 h-14 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shadow-inner group-hover:scale-110 transition-transform">
    ...
    <div class="bg-gradient-to-br from-slate-900 to-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl relative overflow-hidden group hover:border-rose-500/30 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                <span class="block text-xs font-extrabold uppercase tracking-wider text-rose-300">Berisiko Tinggi / Sangat Tinggi</span>
                ...
                <span class="text-[11px] text-slate-400 mt-1 block">Prioritas utama pendampingan</span>
            </div>
            <div class="w-14 h-14 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center text-rose-400 shadow-inner group-hover:scale-110 transition-transform">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Card 3: Total Pengukuran Bulan Ini -->
    <div class="bg-gradient-to-br from-slate-900 to-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl relative overflow-hidden group hover:border-teal-500/30 transition-all duration-300 sm:col-span-2 lg:col-span-1">
        <div class="flex items-center justify-between">
            <div>
                <span class="block text-xs font-extrabold uppercase tracking-wider text-slate-400">Pengukuran Bulan Ini</span>
                ...
                <span class="text-[11px] text-slate-400 mt-1 block">Pemeriksaan fisik {{ now()->translatedFormat('F Y') }}</span>
            </div>
            <div class="w-14 h-14 rounded-2xl bg-teal-500/10 border border-teal-500/30 flex items-center justify-center text-teal-400 shadow-inner group-hover:scale-110 transition-transform">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
    <a href="{{ route('pengukuran.create') }}" class="p-4 bg-gradient-to-r from-emerald-500 to-teal-400 text-slate-950 font-extrabold rounded-2xl shadow-lg hover:shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-between gap-3 min-h-[52px]">
    ...
    <a href="{{ route('ranking.index') }}" class="p-4 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-white font-extrabold rounded-2xl active:scale-95 transition-all flex items-center justify-between gap-3 min-h-[52px]">
        <div class="flex items-center gap-3">
...
<div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Balita dengan Nilai Preferensi V terkecil (Risiko Stunting Tertinggi)</p>
        </div>
        <a href="{{ route('ranking.index') }}" class="text-xs font-bold text-emerald-400 hover:text-emerald-300 inline-flex items-center gap-1 self-start sm:self-auto">
    ...
    @else
        <div class="text-center py-8 text-slate-400 text-xs">
            Belum ada data hasil perhitungan SPK SAW.
        </div>
    @endif
</div>
