...
<div class="space-y-6">
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl space-y-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <span class="text-xs font-bold uppercase tracking-wider text-emerald-400">Posyandu Session</span>
                <h2 class="text-lg sm:text-xl font-black text-white mt-0.5">Input Pengukuran Fisik Bulanan</h2>
                <p class="text-xs text-slate-400 mt-1">Pilih periode pengukuran untuk mencatat atau menimpa (upsert) data tinggi & berat badan balita.</p>
            </div>
            ...
            <form action="{{ route('pengukuran.index') }}" method="GET" class="flex items-center gap-2">
                <div class="relative min-w-[200px]">
                    ...
                    <select id="periode_select" name="periode_key" onchange="submitPeriode(this)" class="w-full bg-slate-950 border border-slate-700 text-white font-extrabold text-xs rounded-2xl px-4 py-3 appearance-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none cursor-pointer pr-10 shadow-inner">
        ...
        <div class="p-4 bg-slate-950/80 border border-slate-800/80 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            <div class="flex items-center gap-3 w-full sm:w-auto">
                ...
                <div>
                    <span class="text-slate-400 text-[11px] block">Status Kelengkapan Pengukuran Periode Ini:</span>
                    <span class="font-black text-white text-sm">
                        <span id="stat_sudah" class="text-emerald-400">{{ $sudahDiukur }}</span> dari {{ $totalBalita }} Balita Sudah Diukur ({{ $percent }}%)
                    </span>
                </div>
            </div>

            <!-- Progress Bar -->
            <div class="w-full sm:w-48 bg-slate-800 rounded-full h-2.5 overflow-hidden border border-slate-700">
    ...
    <div id="ajax_alert" class="hidden p-4 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-xs font-bold flex items-center justify-between gap-2 shadow-lg animate-fade-in">
        ...
        <button onclick="document.getElementById('ajax_alert').classList.add('hidden')" class="text-slate-400 hover:text-white p-1">✕</button>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl space-y-5">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
            <div>
                <h3 class="text-base font-extrabold text-white">Daftar Balita Posyandu</h3>
                <p class="text-xs text-slate-400 mt-0.5">Tekan nama/kartu balita untuk membuka modal input data</p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <div class="flex items-center gap-1.5 bg-slate-950 p-1 rounded-2xl border border-slate-800 shrink-0">
                    <button type="button" id="filter_btn_all" onclick="setStatusFilter('all')" 
                        class="px-3.5 py-1.5 rounded-xl text-xs font-black transition-all min-h-[36px] bg-slate-800 text-white border border-slate-700 shadow-sm">
                        Semua ({{ $totalBalita }})
                    </button>
                    <button type="button" id="filter_btn_belum" onclick="setStatusFilter('belum')" 
                        class="px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all min-h-[36px] text-slate-400 hover:text-slate-200">
        ...
        @if($anaks->count() > 0)
                ...
                @foreach($anaks as $anak)
                    ...
                    <div id="card_anak_{{ $anak->id }}" 
                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <a href="{{ route('balita.show', $anak->id) }}" class="font-black text-white text-sm hover:text-emerald-400 transition-colors block">
                                    {{ $anak->nama }}
                                </a>
            ...
            <div id="no_results_notice" class="hidden text-center py-12 bg-slate-950/60 border border-slate-800/80 rounded-2xl text-slate-400 text-xs space-y-2">
                <svg class="w-8 h-8 mx-auto text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <p class="font-bold text-white text-sm">Tidak ada balita yang ditemukan</p>
...
<div id="inputModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-700 w-full max-w-md rounded-3xl p-5 sm:p-6 shadow-2xl space-y-5 animate-scale-in relative">
        
        <!-- Modal Header -->
        <div class="flex items-center justify-between border-b border-slate-800 pb-4">
            <div>
                <span class="text-[10px] font-black uppercase tracking-wider text-emerald-400">Modal Input Fast UI</span>
                <h3 id="modal_anak_nama" class="text-lg font-black text-white">Nama Balita</h3>
                <p id="modal_sub" class="text-xs text-slate-400 mt-0.5">Periode: {{ \Carbon\Carbon::createFromDate($selectedTahun, $selectedBulan, 1)->translatedFormat('F Y') }}</p>
            </div>
            <button type="button" onclick="closeInputModal()" class="w-9 h-9 rounded-xl bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center border border-slate-700 transition-colors">
                ✕
            </button>
        </div>
        ...
        <form id="modalForm" onsubmit="submitModalForm(event)" class="space-y-4">
            ...
            <div class="space-y-4">
                <!-- Tinggi Badan (cm) -->
                <div>
                    <label for="modal_tinggi_cm" class="block text-xs font-extrabold text-slate-200 mb-1">
                        Tinggi Badan (cm) <span class="text-rose-400">*</span>
                    </label>
                    <div class="relative">
                        <input type="number" step="0.1" min="30" max="150" name="tinggi_cm" id="modal_tinggi_cm" required
                            placeholder="Contoh: 85.5"
                            class="w-full bg-slate-950 border border-slate-700 text-emerald-400 font-extrabold text-base rounded-2xl px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none shadow-inner">
                        <span class="absolute right-4 top-3.5 text-xs font-bold text-slate-400">cm</span>
                    </div>
                </div>

                <!-- Berat Badan (kg) -->
                <div>
                    <label for="modal_berat_kg" class="block text-xs font-extrabold text-slate-200 mb-1">
                        Berat Badan (kg) <span class="text-rose-400">*</span>
                    </label>
                    <div class="relative">
                        <input type="number" step="0.1" min="1" max="40" name="berat_kg" id="modal_berat_kg" required
                            placeholder="Contoh: 11.8"
                            class="w-full bg-slate-950 border border-slate-700 text-teal-400 font-extrabold text-base rounded-2xl px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none shadow-inner">
                        <span class="absolute right-4 top-3.5 text-xs font-bold text-slate-400">kg</span>
                    </div>
                </div>
            </div>

            <!-- Error message container -->
            <div id="modal_error" class="hidden p-3 rounded-xl bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-bold"></div>

            <!-- Submit Button -->
            <div class="pt-3 border-t border-slate-800 flex items-center justify-end gap-3">
                <button type="button" onclick="closeInputModal()" class="py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-xl text-xs min-h-[42px]">
                    Batal
                </button>
...
function setStatusFilter(filterType) {
    ...
    if (filterType === 'all') {
        btnAll.className = 'px-3.5 py-1.5 rounded-xl text-xs font-black transition-all min-h-[36px] bg-slate-800 text-white border border-slate-700 shadow-sm';
        btnBelum.className = 'px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all min-h-[36px] text-slate-400 hover:text-slate-200';
    } else {
        btnBelum.className = 'px-3.5 py-1.5 rounded-xl text-xs font-black transition-all min-h-[36px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 shadow-sm';
        btnAll.className = 'px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all min-h-[36px] text-slate-400 hover:text-slate-200';
    }
