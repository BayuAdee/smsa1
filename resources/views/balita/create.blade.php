...
<div class="max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-8 shadow-xl">
    ...
    <div class="mb-6">
        <h2 class="text-xl font-extrabold text-white">Registrasi Balita Baru</h2>
        <p class="text-xs text-slate-400 mt-1">Lengkapi data identitas fisik awal dan riwayat kelahiran balita.</p>
    </div>
    ...
    <form action="{{ route('balita.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="nama" name="nama" value="{{ old('nama') }}" required 
                   class="w-full bg-slate-950 border border-slate-700 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400 text-white placeholder-slate-500 rounded-2xl p-3.5 text-sm min-h-[48px]"
                   placeholder="Contoh: Muhammad Raihan">
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="nik" class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">NIK (Opsional)</label>
                <input type="text" id="nik" name="nik" value="{{ old('nik') }}" inputmode="numeric"
                       class="w-full bg-slate-950 border border-slate-700 focus:border-emerald-400 text-white placeholder-slate-500 rounded-2xl p-3.5 text-sm min-h-[48px]"
                       placeholder="320101...">
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="berat_lahir_gram" class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Berat Lahir (Gram) <span class="text-rose-400">*</span></label>
        ...
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-300 mb-2">Riwayat BBLR (Berat Lahir Low &lt; 2500g) <span class="text-rose-400">*</span></label>
