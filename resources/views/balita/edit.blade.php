    ...
    <form action="{{ route('balita.update', $anak->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="nik" name="nik" value="{{ old('nik', $anak->nik) }}" inputmode="numeric" maxlength="16"
                       class="w-full bg-slate-50 dark:bg-slate-950 border @error('nik') border-rose-500 @else border-slate-300 dark:border-slate-700 @enderror focus:border-emerald-500 dark:focus:border-emerald-400 text-slate-900 dark:text-white rounded-2xl p-3.5 text-sm min-h-[48px] transition-all"
